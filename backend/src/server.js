...
import { people, cities, companies, communities, interests, skills, livesInLinks, worksAtLinks, memberOfLinks, hasInterestLinks, hasSkillLinks, knowsLinks } from './mockData.js';
const app = express();
...
app.get('/api/people', checkDbConnection, async (req, res) => {
    ...
    if (isFallbackMode()) {
        const filtered = people.filter(p => {
            ...
            const matchCompany = !company || p.company.toLowerCase().includes(company.toLowerCase());
            const matchCity = !city || p.city.toLowerCase().includes(city.toLowerCase());
            const matchCommunity = !community || p.community.toLowerCase().includes(community.toLowerCase());
            const matchSkill = !skill || p.skills.some((s) => s.toLowerCase().includes(skill.toLowerCase()));
            const matchInterest = !interest || p.interests.some((i) => i.toLowerCase().includes(interest.toLowerCase()));
    ...
    try {
       ...
       ORDER BY p.name ASC
       LIMIT 100`, { name, skill, interest, company, city, community });
...
app.get('/api/people/:id', checkDbConnection, async (req, res) => {
    const { id } = req.params;
    if (isFallbackMode()) {
    ...
    const session = driver.session();
    try {
        const personRes = await session.run(`MATCH (p:Person {id: $id})
       OPTIONAL MATCH (p)-[:WORKS_AT]->(co:Company)
       OPTIONAL MATCH (p)-[:LIVES_IN]->(ci:City)
       OPTIONAL MATCH (p)-[:MEMBER_OF]->(comm:Community)
       OPTIONAL MATCH (p)-[:HAS_SKILL]->(sk:Skill)
       OPTIONAL MATCH (p)-[:HAS_INTEREST]->(int:Interest)
       WITH p, co, ci, comm, collect(DISTINCT sk.name) AS skills, collect(DISTINCT int.name) AS interests
       RETURN p {.*, company: co.name, city: ci.name, community: comm.name, skills: skills, interests: interests} AS person`, { id });
        ...
        const connRes = await session.run(`MATCH (p:Person {id: $id})-[r:KNOWS]-(other:Person)
...
app.get('/api/people/:id/mutuals/:otherId', checkDbConnection, async (req, res) => {
    ...
    const session = driver.session();
    try {
        const result = await session.run(`MATCH (p1:Person {id: $id})-[r1:KNOWS]-(m:Person)-[r2:KNOWS]-(p2:Person {id: $otherId})
       ...
       RETURN m {.*, company: co.name, city: ci.name, skills: skills, interests: interests} AS mutual`, { id, otherId });
...
app.get('/api/people/:id/recommendations', checkDbConnection, async (req, res) => {
    const { id } = req.params;
    if (isFallbackMode()) {
        ...
        const recs = people
            ...
            const sharedInterests = p.interests.filter((i) => other.interests.includes(i));
            const interestScore = sharedInterests.length * 5;
            const sharedSkills = p.skills.filter((s) => other.skills.includes(s));
...
app.get('/api/path', checkDbConnection, async (req, res) => {
    ...
    if (isFallbackMode()) {
        ...
        for (let i = 0; i < foundPath.length - 1; i++) {
            ...
            const sharedInterests = u1?.interests.filter((x) => u2?.interests.includes(x)) || [];
            const sharedSkills = u1?.skills.filter((x) => u2?.skills.includes(x)) || [];
            pathEdges.push({
                from: u1Id,
                to: u2Id,
    ...
    const session = driver.session();
    try {
        const pathRes = await session.run(`MATCH (start:Person {id: $from})
       ...
       RETURN path`, { from, to });
        ...
        const nodeIds = [pathRecord.start.properties.id, ...segments.map((seg) => seg.end.properties.id)];
        const detailsRes = await session.run(`MATCH (p:Person)
       ...
       RETURN p {.*, company: co.name, city: ci.name, skills: skills, interests: interests} AS person`, { nodeIds });
        ...
        const edges = segments.map((seg) => {
            ...
            const sharedInterests = u1.interests.filter((i) => u2.interests.includes(i));
            const sharedSkills = u1.skills.filter((s) => u2.skills.includes(s));
            return {
...
app.get('/api/interests', checkDbConnection, async (req, res) => {
    if (isFallbackMode()) {
    ...
    const session = driver.session();
    try {
        const result = await session.run(`MATCH (i:Interest)
...
app.get('/api/interests/:id/people', checkDbConnection, async (req, res) => {
    const { id } = req.params;
    if (isFallbackMode()) {
    ...
    const session = driver.session();
    try {
        const result = await session.run(`MATCH (i:Interest {id: $id})<-[:HAS_INTEREST]-(p:Person)
       OPTIONAL MATCH (p)-[:WORKS_AT]->(co:Company)
       OPTIONAL MATCH (p)-[:LIVES_IN]->(ci:City)
       RETURN p {.*, company: co.name, city: ci.name} AS person`, { id });
        const peopleResult = result.records.map(rec => rec.get('person'));
        res.json(toNative(peopleResult));
    }
    catch (error) {
...
app.get('/api/graph', checkDbConnection, async (req, res) => {
        ...
        if (personId) {
            ...
            if (p) {
                ...
                peers.forEach(peerId => {
                    ...
                    if (peer)
                        addNode(peer.id, peer.name, 'Person', peer);
                });
    ...
    try {
      ...
      `;
        ...
        if (personId && rec) {
            const p = rec.get('p');
            if (p)
                addNode(p.properties.id, p.properties.name, 'Person', p.properties);
            rec.get('peers').forEach((peer) => {
                if (peer)
                    addNode(peer.properties.id, peer.properties.name, 'Person', peer.properties);
            });
            rec.get('companies').forEach((co) => {
                if (co)
                    addNode(co.properties.id, co.properties.name, 'Company');
            });
            rec.get('cities').forEach((ci) => {
                if (ci)
                    addNode(ci.properties.id, ci.properties.name, 'City');
            });
            rec.get('communities').forEach((comm) => {
                if (comm)
                    addNode(comm.properties.id, comm.properties.name, 'Community');
            });
        }
        else if (interestId && rec) {
            const iNode = rec.get('i');
            if (iNode)
                addNode(iNode.properties.id, iNode.properties.name, 'Interest');
            rec.get('people').forEach((p) => {
                if (p)
                    addNode(p.properties.id, p.properties.name, 'Person', p.properties);
            });
            rec.get('companies').forEach((co) => {
        ...
        const edgeRes = await session.run(`MATCH (n)-[r]->(m)
       WHERE n.id IN $nodeIds AND m.id IN $nodeIds
       RETURN id(r) AS relId, n.id AS source, m.id AS target, type(r) AS type`, { nodeIds });
