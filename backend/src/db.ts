...
let lastCheckTime = 0;
let cachedStatus: { connected: boolean; error?: string; fallback?: boolean } | null = null;

export async function checkConnection(): Promise<{ connected: boolean; error?: string; fallback?: boolean }> {
  const now = Date.now();
